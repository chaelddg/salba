...
// DELETE REQUEST
exports.adminReturnBookRequestPOST = function (req, res) {
  console.log("*************** decline **************");
  console.log(req.body);
  async.waterfall([
      function(callback) {
      ...
      function(arg1, callback) {
        AdminLogs.findOne({requestid: ObjectID(arg1._id)}).exec(function(err, log) {
          console.log('**************** DELETE REQUEST 2 ***************');
          console.log(log);
          callback(null, arg1, log);
        });
      },
      function(arg1, arg2, callback) {
        var returned_date = new Date(moment());
        AdminLogs.findByIdAndUpdate(ObjectID(arg2._id), { $set: { returned: returned_date }}, function (err, logdata) {
            if (err) console.log(err);
            callback(null, arg1);
        });
      },
